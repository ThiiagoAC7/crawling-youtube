...
class YouTubeAPIManager:
    ...
    def get_new_youtube_client(self):
        ...
        # Verifica se já tentou todas as chaves
        if self.current_key_index >= len(self.DEVELOPER_KEYS) - 1:
            # timeout = secondsUntil(5)
            timeout = 60
            log("key",
                f"Todas as chaves excederam a quota. Aguardando {timeout} segundos.")
            ...
            while True:
                try:
                    ...
                    if razao == "quotaExceeded":
                        ...
                        # Obtém um novo cliente com a próxima chave
                        self.youtube = self.get_new_youtube_client()
                        log("key",
                            f"Chave atual: {self.current_key_index + 1}/{len(self.DEVELOPER_KEYS)}")
                        ...
                        print(e)
                        print(
                            "Acesso ao vídeo é restrito e requer autorização adicional.")
                        return None
                    else:
                        # log("error", e)
                        print("Erro ao enviar requisição para a API do YouTube - Tentando novamente em ",
                              config["try_again_timeout"], "s")
                        time.sleep(config["try_again_timeout"])

                except Exception as e:
                    try_again_timeout = config["try_again_timeout"]
                    print(
                        f"Problema de conexão com o YouTube detectado - Tentando novamente em {try_again_timeout} segundos.")
...
def generate_date_intervals(start_date, end_date, interval_type):
    interval_delta = {"weekly": timedelta(
        weeks=1), "monthly": timedelta(days=30)}
    current_start = end_date
    while current_start > start_date:
        current_end = min(current_start, end_date)
        current_start = max(
            current_end - interval_delta[interval_type], start_date)
...
def get_comments(video_id, video_title, total_comment_count):
    ...
    while True:
        try:
            method_func = lambda client, **kwargs: api_manager.youtube.commentThreads().list(**
                                                                                             kwargs)
            ...
            if e.resp.status == 404:
                log("video",
                    f"Vídeo com ID {video_id} não encontrado. Pulando...")
                return comments_data  # Retorna os dados de comentários coletados até agora
            elif razao == "commentsDisabled":
                log("video",
                    f"Vídeo com ID {video_id} possui comentários bloqueados. Pulando...")
                return comments_data  # Retorna os dados de comentários coletados até agora
            else:
                print(e["detailes"])
        ...
        for item in response.get("items", []):
            ...
            comments_data.append({
                ...
                # Pode não estar presente em todas as respostas
                "updated_at": comment_info.get("updatedAt", None),
    ...
    print(
        f"Coletados {collected_comments} de {total_comment_count} comentários para o vídeo {video_id}.")
...
def get_replies(video_id, comment_id):
    ...
    while True:
        ...
        try:
            ...
            print("Salvando ", len(response.get("items", [])),
                  " replies únicos", video_id)
...
def process_video(video_id, video_title, processed_videos):
    ...
    if video_details == None:
        ...
        return
    # Assumindo que 'comment_count' é o total de comentários disponíveis
    total_comment_count = video_details['comment_count']

    if total_comment_count > 0 and total_comment_count < 10000000000000:  # Sentinel
        pd.DataFrame([video_details]).to_csv('files/videos_info.csv',
                                             mode='a', header=not videos_file_exists, index=False)

        channel_details = get_channel_details(video_details['channel_id'])
        pd.DataFrame([channel_details]).to_csv('files/channels_info.csv',
                                               mode='a', header=not channels_file_exists, index=False)

        comments = get_comments(video_id, video_title, total_comment_count)
        comments_df = pd.DataFrame(comments)
        comments_df['channel_id'] = video_details['channel_id']
        comments_df.to_csv('files/comments_info.csv', mode='a',
                           header=not comments_file_exists, index=False)
...
def make_search_request(query, published_after, published_before, REGION_CODE, RELEVANCE_LANGUAGE):
    ...
    # if youtuber != any 
    # if q == "any", make_api_request sem q, com youtuber
...
def main():
    ...
    start_date = datetime(config['start_date'][0], config['start_date']
                          [1], config['start_date'][2])  # Data inicial da coleta
    end_date = datetime(
        atual_date["year"], atual_date["month"], atual_date["day"])  # Data final
    # Intervalo da busca, se é mensal(monthtly), ou semanal (weekly)
    interval_type = "monthly"
    REGION_CODE = config['region_code']
    RELEVANCE_LANGUAGE = config['relevance_language']
    # Pegar os vídeos mais comentados? Não vale a pena porque está retornando vídeo do tema...
    TOP_COMMENTED = False
    number_of_videos_to_process = 0
    # Forçar o processamento dos vídeos e comentários com determinadas keywords nos títulos...
    ...
    for start_interval, end_interval in generate_date_intervals(start_date, end_date, interval_type):
        ...
        for query in queries:
            ...
            GlobalState.get_instance().set_state(
                "query_progress", f"{queries.index(query) + 1}/{len(queries)}")

            published_after = start_interval.isoformat() + "Z"
            published_before = end_interval.isoformat() + "Z"
            video_details_list = []
            search_response = make_search_request(
                query, published_after, published_before, REGION_CODE, RELEVANCE_LANGUAGE)
            ...
            for index, item in enumerate(videos, start=1):
                ...
                if any((word.lower() in VIDEO_TITLE for word in key_words) or len(key_words) == 0):
                    ...
                    print(
                        f"Processando vídeo {index} de {total_videos}: ID = {video_id}")
                    if video_id not in processed_videos:
                        video_details = get_video_details(video_id)
                        comment_count = video_details['comment_count']
                        print(
                            "Title:", video_details['title'], "# comments", video_details['comment_count'])

                        if comment_count > 0:
                            process_video(video_id, "", processed_videos)

            log("search",
                f"Coleta concluída para a consulta: {query} entre {start_interval} e {end_interval}")
